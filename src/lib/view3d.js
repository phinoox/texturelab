...
export class View3D {
    constructor() {
        ...
        this.repeat = 1;
        // geometry
        this.sphereGeom = new SphereGeometry(0.7, 64, 64);
        this.cubeGeom = new THREE.BoxGeometry();
        this.planeGeom = new THREE.PlaneGeometry(1, 1, 100, 100);
        this.cylinderGeom = new CylinderGeometry(0.5, 0.5, 1, 64, 64, true);
    }
    setCanvas(el) {
        this.setupRenderer(el);
        this.camera = new THREE.PerspectiveCamera(45, el.width / el.height, 0.1, 1000);
    ...
    _init() { }
    setTexture(imageCanvas, channelName) {
    ...
    setAlbedoTexture(imageCanvas, channelName) {
    ...
    setNormalTexture(imageCanvas, channelName) {
        var tex = new THREE.CanvasTexture(imageCanvas.canvas);
        tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
        tex.repeat.set(this.repeat, this.repeat);
        tex.anisotropy = this.renderer.capabilities.getMaxAnisotropy();
    ...
    setMetalnessTexture(imageCanvas, channelName) {
        var tex = new THREE.CanvasTexture(imageCanvas.canvas);
        tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
        tex.repeat.set(this.repeat, this.repeat);
        tex.anisotropy = this.renderer.capabilities.getMaxAnisotropy();
    ...
    setRoughnessTexture(imageCanvas, channelName) {
        var tex = new THREE.CanvasTexture(imageCanvas.canvas);
        tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
        tex.repeat.set(this.repeat, this.repeat);
        tex.anisotropy = this.renderer.capabilities.getMaxAnisotropy();
    ...
    setHeightTexture(imageCanvas, channelName) {
        var tex = new THREE.CanvasTexture(imageCanvas.canvas);
        tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
        tex.repeat.set(this.repeat, this.repeat);
        tex.anisotropy = this.renderer.capabilities.getMaxAnisotropy();
    ...
    setModel(modelName) {
        ...
        let geom;
        if (modelName == "sphere")
            geom = this.sphereGeom;
        if (modelName == "cube")
            geom = this.cubeGeom;
        if (modelName == "plane")
            geom = this.planeGeom;
        if (modelName == "cylinder")
            geom = this.cylinderGeom;
    ...
    loadEnv() {
        ...
        var basePath = (process.env.NODE_ENV == "production" ? "file://" : "") +
